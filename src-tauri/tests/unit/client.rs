...
fn connect_succeeds_with_valid_socket_path() {
    ...
    assert!(
        result.is_ok(),
        "connect should succeed with valid socket path"
    );
}

#[test]
fn client_is_cloneable() {
    let client = DockerClient::connect("/var/run/docker.sock").expect("Failed to create client");
...
fn multiple_clients_are_independent() {
    let client1 = DockerClient::connect("/var/run/docker.sock").expect("Failed to create client1");

    let client2 = DockerClient::connect("/var/run/docker.sock").expect("Failed to create client2");
